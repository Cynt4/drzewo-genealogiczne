...
FamilyTree.templates.ramka.field_1 =
  '<text data-text-overflow="ellipsis" width="240" style="font-size: 12px;" fill="#475569" x="130" y="70" text-anchor="middle">{val}</text>';
FamilyTree.templates.ramka.field_2 =
  '<text data-text-overflow="ellipsis" width="240" style="font-size: 11px;" fill="#64748b" x="130" y="90" text-anchor="middle">{val}</text>';
FamilyTree.templates.ramka.field_3 =
  '<text style="font-size: 11px; font-weight: bold;" fill="#94a3b8" x="250" y="20" text-anchor="end">{val}</text>';
...
function renderTree() {
  ...
  klan.forEach((osoba) => {
    ...
    if (osoba.ojciecId && wszystkieIds.includes(String(osoba.ojciecId)))
      node.fid = String(osoba.ojciecId);
